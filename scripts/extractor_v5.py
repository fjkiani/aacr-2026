#!/usr/bin/env python3
"""
AACR 2026 Full Extraction Pipeline v5
Primary: openai/gpt-oss-120b:free
Fallback: google/gemma-4-31b-it:free
Dual-key rotation, file-based resume, auto-chunking
"""
...
MAX_TOKENS  = 8000
TEMPERATURE = 0.1
CALL_SLEEP  = 10   # seconds between calls
MAX_RETRIES = 6
...
# Run log in shared-workspace (supports append from background)
RUN_LOG = Path("/mnt/shared-workspace/run_log_v5.jsonl")
...
print(f"Total={total} | DoneA={len(done_a)} | DoneB={len(done_b)} | Remaining={total-max(len(done_a),len(done_b))}")
...
for idx, (slug, title) in enumerate(sessions):
    ...
    # Write log to shared-workspace (survives machine death)
    try:
    ...
    if (idx + 1) % 10 == 0:
        ...
        print(f"\n  ═══ A={da}/{total}  B={db}/{total} ═══\n")

print(f"\n✅ Done! A={len(list(OUT_DIR_A.glob('*.json')))} B={len(list(OUT_DIR_B.glob('*.json')))}")
